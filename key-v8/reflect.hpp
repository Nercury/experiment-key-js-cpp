...
#define FLECT_GS(class_definition, property_name, get_accessor, get_definition, get_field, set_accessor, set_definition, set_field, doc_type_name, doc_text) \
	static void reflect_ ## property_name ## (cvv8::ClassCreator<class_definition> & cc, v8::Handle<v8::ObjectTemplate> proto, bool for_static) { \
		...
		if (for_static) { \
			cc.CtorFunction()->SetAccessor(JSTR(#property_name), \
				cvv8::FunctionToGetter< std::map<std::string,std::string> (), & ## class_definition ## :: ## get ## property_name ## _reflection ## >::Get \
			); \
		} \
	} \
	static std::map<std::string,std::string> get ## property_name ## _reflection() { \
...
#define REFLECT(name) reflect_ ## name(cc, proto, for_static); \
